...
export default function Filter() {
  ...
  const filter = (button) => {
    ...
    const filteredData = resort.filter((data) => data.location == button);
  ...
  return (
    <>
      <div className="h-[25%] w-[80%] ">
        <select className="w-[20%] h-full drop-shadow-xl text-black text-[20px] rudaw-font text-center rounded-md focus:outline-none">
          <option
            value="slemani"
            onClick={() => {
              filter("all");
