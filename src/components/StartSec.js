...
export default function StartSec() {
  return (
    <div className="w-[85%] h-[60%] mx-auto sm:w-full md:w-full">
      ...
      <input  className="w-[70%] sm:w-full lg:w-[70%] text-black h-[15%] p-[2.5%]  focus:outline-none hover:shadow-insetShadow shadow-md rounded-3xl placeholder:rudaw-font" icon="search" placeholder="گەڕان.."/>
      </div>
      ...
      <div className="w-full h-[75%] flex items-center justify-around " >
       {
        content.map((content,index)=>{
         return(
          <div key={index} className="Card w-[35%] h-[60%] rounded-md ">
          <img src={content.img} alt="img"  className=" rounded-tl-md rounded-tr-md"/>
          <h6 className="w-full rudaw-font pt-[1%] text-black bg-white text-[20px] text-center flex items-center justify-center  shadow-md hover:shadow-lg">{content.title}</h6>
        </div>
         )
        })
       }
